...
data Symbol = Var String                -- ^ Variablenname
            | Fun1 String Symbol        -- ^ Funktionsame Parameter
            | Fun2 String Symbol Symbol -- ^ Funktionsname Parameter1 Parameter2
            deriving (Ord, Eq)
...
data Equality = Equal Symbol Symbol
              | NonEq Symbol Symbol
              deriving (Ord)
...
compareEquality e1 e2 = undefined
...
merge
  :: Equivalence    -- ^ Die Äquivalenzrelation
  -> EqClass        -- ^ Die erste Klasse der Relation
  -> EqClass        -- ^ Die zweite Klasse der Relation
  -> Equivalence    -- ^ Die neue Äquivalenzrelation mit den vereinigten Klassen
merge (Equivalence eq) ec1@(EqClass e1) ec2@(EqClass e2) =
  Equivalence (newClass `Set.insert` eq')
  where eq' = ec2 `Set.delete` (ec1 `Set.delete` eq)
...
getClass (Equivalence eq) s =
  ...
  where classes = Set.filter (\(EqClass c) -> s `Set.member` c) eq
...
mkEqClass s@(Fun2 _ t1 t2) = Set.insert (EqClass (Set.singleton s))
...
congruenceClosure eq = go eq
  where functionSymbols = symbolsEquivalence eq
        ...
        --
        mergeFuncClasses
          :: [Symbol]      -- ^ Liste von Werten vom Typ 'Symbol'
          -> [Symbol]      -- ^ Liste von Werten vom Typ 'Symbol'
          -> Equivalence   -- ^ die Eingabe Äquivalenzrelation
          -> Equivalence   -- ^ die berechnete neue Äquivalenzrelation
        ...
        mergeFuncClasses (s@(Fun1 n1 t1):rest) (s'@(Fun1 n2 t2):rest') eqv =
          ...
          where eqv' = merge eqv (getClass eqv s) (getClass eqv s')

---------------
-- Aufgabe 2 --
---------------

        mergeFuncClasses ((Fun2 _ _ _):rest1) ((Fun2 _ _ _):rest2) eqv = undefined
...
initialEquivalence ((Equal s1 s2):rest) (Equivalence eq) =
  ...
  where eq' = mkEqClass s1 `Set.union` (mkEqClass s2 `Set.union` eq)
...
buildEquivalence ((Equal s1 s2):rest) eq =
  ...
  where eq' = merge eq (getClass eq s1) (getClass eq s2)
